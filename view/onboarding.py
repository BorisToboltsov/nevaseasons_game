...
async def start_game_participant(message: Message,
                                 user_telegram_id: int) -> NoReturn:
    context = f"Для начала игры нажмите старт"
    await message.bot.send_message(user_telegram_id, context, reply_markup=await get_inline_keyboard('Старт'))

...
async def button_no(
    ...
    if task.template.question.type_response == 1:
        await bg.switch_to(state=FSMStartGame.choice_no_answer)
    elif task.template.question.type_response == 2:
        await bg.switch_to(state=FSMStartGame.input_text_no_answer)
    elif task.template.question.type_response == 3:
        await bg.switch_to(state=FSMStartGame.input_text_photo_no_answer)
    await dialog_manager.done()
    await callback.message.delete()
